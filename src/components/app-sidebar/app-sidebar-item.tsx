...
export function AppSidebarItem({ icon, ...props }: ItemPropsType) {
  ...
  return (
    <li
      className="hover:bg-sidebar-accent border-border flex size-16 cursor-pointer items-center justify-center rounded-md hover:border-1"
      {...props}
    >
...
export function AppSidebarLinkItem({ icon, path }: LinkItemPropsType) {
  ...
  return (
    <Link to={path}>
      <li
        className={cn(
          "hover:bg-sidebar-accent border-border flex size-16 cursor-pointer items-center justify-center rounded-md hover:border-1",
